...
pub fn refund_deposit_to_account(storage_used: u64, account_id: AccountId) {
    ...
    require!(
        ...
        format!("Must attach {} yoctoUNC to cover storage", required_cost)
    );
...
/// Assert that at least 1 yoctoUNC was attached.
pub(crate) fn assert_at_least_one_atto() {
    require!(
        env::attached_deposit() >= UncToken::from_attounc(1),
        "Requires attached deposit of at least 1 yoctoUNC"
    )
}
