...
///
/// ```
/// # let context = unc_sdk::test_utils::VMContextBuilder::new().build();
/// # let vm_config = unc_sdk::test_vm_config();
/// # let fees_config = unc_sdk::RuntimeFeesConfig::test();
/// # let storage = Default::default();
/// # let validators = Default::default();
/// let mocked_blockchain = unc_sdk::MockedBlockchain::new(
///           context,
///           vm_config,
///           fees_config,
///           vec![],
...
///           None,
///       );
/// unc_sdk::env::set_blockchain_interface(mocked_blockchain);
/// ```
#[cfg(all(not(target_arch = "wasm32"), feature = "unit-testing"))]
...
/// ```
///
/// Example of usage with [unc-rng](https://lib.rs/crates/unc-rng) which allows to decrease size of contract binary:
///
/// ```rust
/// use unc_rng::Rng;
/// use unc_sdk::unc;
...
/// }
/// ```
/// More info in [documentation](https://docs.unc.org/develop/contracts/security/random)
...
pub fn promise_batch_action_transfer(promise_index: PromiseIndex, amount: UncToken) {
    unsafe {
...
pub(crate) fn migrate_to_allowance(allowance: UncToken) -> Allowance {
...
pub fn validator_stake(account_id: &AccountId) -> UncToken {
    let account_id: &str = account_id.as_ref();
    let data = [0u8; size_of::<UncToken>()];
    unsafe {
...
///
/// ```
/// use unc_sdk::env::{storage_write, storage_read};
///
...
mod tests {
    ...
    fn test_is_valid_account_id_binary() {
        ...
        assert!(is_valid_account_id(b"unc"));
    }
