...
use unc_sdk::{unc_bindgen, AccountId};

#[unc_bindgen(event_json(standard = "test_standard"))]
pub enum TestEvents<'a, 'b, T>
where
    T: unc_sdk::serde::Serialize,
{
    #[event_version("1.0.0")]
...
mod private {
    ...
    #[unc_bindgen(event_json(standard = "another_standard"))]
...
#[test]
fn test_json_emit() {
    let token_in: AccountId = "wrap".parse().unwrap();
    let token_out: AccountId = "test".parse().unwrap();
    ...
    assert_eq!(
        logs[0],
        r#"EVENT_JSON:{"standard":"test_standard","version":"1.0.0","event":"swap","data":{"token_in":"wrap","token_out":"test","amount_in":100,"amount_out":200,"test":"tst"}}"#
    );
    assert_eq!(
        logs[1],
    ...
    assert_eq!(
        logs[3],
        r#"EVENT_JSON:{"standard":"test_standard","version":"1.3.0","event":"lifetime_test_a","data":"lifetime"}"#
    );
    assert_eq!(
        logs[4],
        r#"EVENT_JSON:{"standard":"test_standard","version":"2.0.0","event":"lifetime_test_b","data":"lifetime_b"}"#
    );
    assert_eq!(
        logs[5],
