...
fn ensure_abi_for_prepended_functions() {
    ...
    let res = Command::new(cargo)
        ...
        .args(["--features", "unc-sdk/__abi-generate"])
    ...
    let unc_abi_symbols = symbolic_debuginfo::Object::parse(&dylib_file_contents)
        .expect("unable to parse dylib")
        .symbols()
        .flat_map(|sym| sym.name)
        .filter(|sym_name| sym_name.starts_with("__unc_abi_"))
        .collect::<HashSet<_>>();

    // ensure methods are prepended
    PREPENDED_METHODS.map(|method| {
        assert!(
            unc_abi_symbols.contains(format!("__unc_abi_{}", method).as_str()),
