...
struct UncMacroArgs {
    ...
    inside_uncsdk: Option<bool>,
}

/// This attribute macro is used to enhance the unc_bindgen macro.
...
/// use unc_sdk_macro::UncSchema;
/// #[derive(BorshSerialize, BorshDeserialize, Serialize, Deserialize, UncSchema)]
...
/// }
/// ```
/// Please note that `BorshSchema` and `JsonSchema` are added inside UncSchema whenever you use unc macro for struct or enum.
...
/// ```ignore
/// #[unc_bindgen]
/// #[derive(BorshSerialize, BorshDeserialize, UncSchema)]
/// #[borsh(crate = "unc_sdk::borsh")]
/// struct MyStruct {
///    pub name: String,
/// }
...
pub fn unc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let unc_macro_args = match UncMacroArgs::from_list(&meta_list) {
        Ok(v) => v,
        Err(e) => {
    ...
    let unc_sdk_crate = if unc_macro_args.inside_uncsdk.unwrap_or(false) {
        quote! {crate}
    } else {
        quote! {::unc_sdk}
    };
    let string_borsh_crate = quote! {#unc_sdk_crate::borsh}.to_string();
    let string_serde_crate = quote! {#unc_sdk_crate::serde}.to_string();

    let mut expanded: proc_macro2::TokenStream = quote! {};

    if unc_macro_args.contract_state.unwrap_or(false) {
        if let Some(metadata) = unc_macro_args.contract_metadata {
            expanded = quote! {#[#unc_sdk_crate::unc_bindgen(#metadata)]}
        } else {
            expanded = quote! {#[#unc_sdk_crate::unc_bindgen]}
        }
    };
    ...
    match unc_macro_args.serializers {
        Some(serializers) => {
    ...
    #[cfg(feature = "abi")]
    {
        let schema_derive: proc_macro2::TokenStream =
            get_schema_derive(has_json, has_borsh, unc_sdk_crate.clone(), false);
        expanded = quote! {
            #expanded
            #schema_derive
        };
    ...
    if has_borsh {
        expanded = quote! {
            #expanded
            #[derive(#unc_sdk_crate::borsh::BorshSerialize, #unc_sdk_crate::borsh::BorshDeserialize)]
            #borsh_attr
        };
    }
    ...
    if has_json {
        expanded = quote! {
            #expanded
            #[derive(#unc_sdk_crate::serde::Serialize, #unc_sdk_crate::serde::Deserialize)]
    ...
    if let Ok(input) = syn::parse::<ItemStruct>(item.clone()) {
        ...
        return TokenStream::from(
            syn::Error::new(
                Span::call_site(),
                "unc macro can only be used on struct or enum definition and impl sections.",
            )
            .to_compile_error(),
        );
    }
...
#[proc_macro_derive(UncSchema, attributes(abi, serde, borsh, schemars, validate, inside_uncsdk))]
pub fn derive_unc_schema(#[allow(unused)] input: TokenStream) -> TokenStream {
    ...
    #[cfg(feature = "abi")]
    {
        ...
        let unc_sdk_crate =
            if derive_input.attrs.iter().any(|attr| attr.path().is_ident("inside_uncsdk")) {
                quote! {crate}
            } else {
                quote! {::unc_sdk}
            };
        ...
        let derive = get_schema_derive(json_schema, borsh_schema, unc_sdk_crate.clone(), true);

        let input_ident = &input.ident;

        let input_ident_proxy = quote::format_ident!("{}__UNC_SCHEMA_PROXY", input_ident);

        let json_impl = if json_schema {
            let where_clause = get_where_clause(
                &generics,
                input_ident,
                quote! {#unc_sdk_crate::schemars::JsonSchema},
            );
            quote! {
                #[automatically_derived]
                impl #generics #unc_sdk_crate::schemars::JsonSchema for #input_ident_proxy #generics #where_clause {
                    fn schema_name() -> ::std::string::String {
                        stringify!(#input_ident #generics).to_string()
                    }

                    fn json_schema(gen: &mut #unc_sdk_crate::schemars::gen::SchemaGenerator) -> #unc_sdk_crate::schemars::schema::Schema {
                        <#input_ident #generics as #unc_sdk_crate::schemars::JsonSchema>::json_schema(gen)
                    }
                }
            }
        } else {
        ...
        let borsh_impl = if borsh_schema {
            let where_clause = get_where_clause(
                &generics,
                input_ident,
                quote! {#unc_sdk_crate::borsh::BorshSchema},
            );
            quote! {
                #[automatically_derived]
                impl #generics #unc_sdk_crate::borsh::BorshSchema for #input_ident_proxy #generics #where_clause {
                    fn declaration() -> #unc_sdk_crate::borsh::schema::Declaration {
                        stringify!(#input_ident #generics).to_string()
                    }

                    fn add_definitions_recursively(
                        definitions: &mut #unc_sdk_crate::borsh::__private::maybestd::collections::BTreeMap<
                            #unc_sdk_crate::borsh::schema::Declaration,
                            #unc_sdk_crate::borsh::schema::Definition
                        >,
                    ) {
                        <#input_ident #generics as #unc_sdk_crate::borsh::BorshSchema>::add_definitions_recursively(definitions);
                    }
                }
            }
        } else {
...
fn get_schema_derive(
    json_schema: bool,
    borsh_schema: bool,
    unc_sdk_crate: proc_macro2::TokenStream,
    need_borsh_crate: bool,
) -> proc_macro2::TokenStream {
    let string_borsh_crate = quote! {#unc_sdk_crate::borsh}.to_string();
    let string_schemars_crate = quote! {#unc_sdk_crate::schemars}.to_string();

    let mut derive = quote! {};
    if borsh_schema {
        derive = quote! {
            #[cfg_attr(not(target_arch = "wasm32"), derive(#unc_sdk_crate::borsh::BorshSchema))]
        };
        if need_borsh_crate {
            derive = quote! {
                #derive
    ...
    if json_schema {
        derive = quote! {
            #derive
            #[cfg_attr(not(target_arch = "wasm32"), derive(#unc_sdk_crate::schemars::JsonSchema))]
