...
pub fn generate(i: &ItemImplInfo) -> TokenStream2 {
    ...
    let unc_abi_symbol = format_ident!("__unc_abi_{}", first_function_name);
    quote! {
        #[cfg(not(target_arch = "wasm32"))]
        const _: () = {
            #[no_mangle]
            pub extern "C" fn #unc_abi_symbol() -> (*const u8, usize) {
                use ::std::string::String;

                let mut gen = ::unc_sdk::schemars::gen::SchemaGenerator::default();
                let functions = vec![#(#functions),*];
                let mut data = ::std::mem::ManuallyDrop::new(
                    ::unc_sdk::serde_json::to_vec(&::unc_sdk::__private::ChunkedAbiEntry::new(
                        functions,
...
impl ImplItemMethodInfo {
    ...
    /// ```ignore
    /// unc_sdk::__private::AbiFunction {
    ///     name: "f3".to_string(),
    ///     doc: Some(" I am a function.".to_string()),
    ///     kind: unc_sdk::__private::AbiFunctionKind::Call,
    ///     modifiers: vec![],
    ///     params: unc_sdk::__private::AbiParameters::Json {
    ///         args: vec![
    ///             unc_sdk::__private::AbiJsonParameter {
    ///                 name: "arg0".to_string(),
    ...
    ///     callbacks: vec![],
    ///     callbacks_vec: None,
    ///     result: Some(unc_sdk::__private::AbiType::Json {
    ///         type_schema: gen.subschema_for::<IsOk>(),
    ///     })
    /// }
    /// ```
    /// If args are serialized with Borsh it will not include `#[derive(::unc_sdk::borsh::BorshSchema)]`.
    pub fn abi_struct(&self) -> TokenStream2 {
        ...
        let kind = match &attr_signature_info.method_kind {
            MethodKind::View(_) => quote! { ::unc_sdk::__private::AbiFunctionKind::View },
            MethodKind::Call(_) => {
                quote! { ::unc_sdk::__private::AbiFunctionKind::Call }
            }
            MethodKind::Init(_) => {
                modifiers.push(quote! { ::unc_sdk::__private::AbiFunctionModifier::Init });
                quote! { ::unc_sdk::__private::AbiFunctionKind::Call }
            }
        };
        if attr_signature_info.is_payable() {
            modifiers.push(quote! { ::unc_sdk::__private::AbiFunctionModifier::Payable });
        }
        if attr_signature_info.is_private() {
            modifiers.push(quote! { ::unc_sdk::__private::AbiFunctionModifier::Private });
        }
        let modifiers = quote! {
            ::std::vec![#(#modifiers),*]
        };
                ...
                BindgenArgType::Regular => {
                    ...
                    match arg.serializer_ty {
                        ...
                        SerializerType::Borsh => params.push(quote! {
                            ::unc_sdk::__private::AbiBorshParameter {
                                name: ::std::string::String::from(#arg_name),
                                type_schema: #schema,
                            }
                        }),
        ...
        let params = match attr_signature_info.input_serializer {
            SerializerType::JSON => quote! {
                ::unc_sdk::__private::AbiParameters::Json {
                    args: ::std::vec![#(#params),*]
                }
            },
            SerializerType::Borsh => quote! {
                ::unc_sdk::__private::AbiParameters::Borsh {
                    args: ::std::vec![#(#params),*]
                }
            },
        };
    ...
    fn abi_result_tokens(&self) -> TokenStream2 {
        ...
        match &self.attr_signature_info.returns.kind {
            ...
            HandlesResult(ty) => {
                ...
                let ty = parse_quote! { <#ty as unc_sdk::__private::ResultTypeExt>::Okay };
...
fn generate_schema(ty: &Type, serializer_type: &SerializerType) -> TokenStream2 {
    match serializer_type {
        ...
        SerializerType::Borsh => quote! {
            ::unc_sdk::borsh::schema_container_of::<#ty>()
        },
    }
}
...
fn generate_abi_type(ty: &Type, serializer_type: &SerializerType) -> TokenStream2 {
    ...
    match serializer_type {
        SerializerType::JSON => quote! {
            ::unc_sdk::__private::AbiType::Json {
                type_schema: #schema,
            }
        },
        SerializerType::Borsh => quote! {
            ::unc_sdk::__private::AbiType::Borsh {
                type_schema: #schema,
            }
        },
    }
