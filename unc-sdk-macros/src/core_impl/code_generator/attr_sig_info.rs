            ...
            .fold(TokenStream2::new(), |acc, (idx, arg)| {
                ...
                match &bindgen_ty {
                    BindgenArgType::CallbackArg => {
                        ...
                        let read_data = quote! {
                            let data: ::std::vec::Vec<u8> = match ::unc_sdk::env::promise_result(#idx) {
                                ::unc_sdk::PromiseResult::Successful(x) => x,
                                _ => ::unc_sdk::env::panic_str(#error_msg)
                            };
                        };
                    ...
                    BindgenArgType::CallbackResultArg => {
                        ...
                        let deserialization_branch = match ok_type {
                            ...
                            syn::Type::Tuple(type_tuple) if type_tuple.elems.is_empty() =>
                                quote! {
                                    ::unc_sdk::PromiseResult::Successful(data) if data.is_empty() =>
                                        ::std::result::Result::Ok(()),
                                    ::unc_sdk::PromiseResult::Successful(data) => ::std::result::Result::Ok(#deserialize)
                                },
                            _ =>
                                quote! {
                                    ::unc_sdk::PromiseResult::Successful(data) => ::std::result::Result::Ok(#deserialize)
                        ...
                        let result = quote! {
                            match ::unc_sdk::env::promise_result(#idx) {
                                #deserialization_branch,
                                ::unc_sdk::PromiseResult::Failed => ::std::result::Result::Err(::unc_sdk::PromiseError::Failed),
                            }
                        };
                        quote! {
                            #acc
                    ...
                    let #mutability #ident: #ty = ::std::iter::Iterator::collect(::std::iter::Iterator::map(
                        ...
                        |i| {
                            let data: ::std::vec::Vec<u8> = match ::unc_sdk::env::promise_result(i) {
                                ::unc_sdk::PromiseResult::Successful(x) => x,
                                _ => ::unc_sdk::env::panic_str(&::std::format!("Callback computation {} was not successful", i)),
                            };
                            #invocation
                        }));
                }
...
fn deserialize_data(ty: &SerializerType) -> TokenStream2 {
    match ty {
        SerializerType::JSON => quote! {
            ::unc_sdk::serde_json::from_slice(&data).expect("Failed to deserialize callback using JSON")
        },
        SerializerType::Borsh => quote! {
            ::unc_sdk::borsh::BorshDeserialize::try_from_slice(&data).expect("Failed to deserialize callback using Borsh")
        },
    }
}
