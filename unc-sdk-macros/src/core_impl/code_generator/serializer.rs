...
pub fn generate_serializer(
    ...
    let value_ser = match serializer {
        SerializerType::JSON => quote! {
            ::unc_sdk::serde_json::to_vec(&__args).expect("Failed to serialize the cross contract args using JSON.")
        },
        SerializerType::Borsh => quote! {
            ::unc_sdk::borsh::to_vec(&__args).expect("Failed to serialize the cross contract args using Borsh.")
        },
    };
