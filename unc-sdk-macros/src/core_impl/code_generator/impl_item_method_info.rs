...
impl ImplItemMethodInfo {
    ...
    fn contract_init_tokens(&self) -> TokenStream2 {
        ...
        let contract_deser = |receiver: &Receiver| {
            ...
            quote! {
                let #mutability contract: #struct_type = ::unc_sdk::env::state_read().unwrap_or_default();
            }
        };
    ...
    fn value_ser_tokens(&self) -> TokenStream2 {
        ...
        let value_ser = |result_serializer: &SerializerType| match result_serializer {
            SerializerType::JSON => quote! {
                let result = ::unc_sdk::serde_json::to_vec(&result).expect("Failed to serialize the return value using JSON.");
            },
            SerializerType::Borsh => quote! {
                let result = ::unc_sdk::borsh::to_vec(&result).expect("Failed to serialize the return value using Borsh.");
            },
        };
